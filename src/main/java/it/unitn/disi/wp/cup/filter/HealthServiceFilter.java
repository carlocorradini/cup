...
/**
 * Filter that check if the {@link HealthService Health Service} is authenticated (and authorized)
 *
 * @author Carlo Corradini
 */
@WebFilter(
        urlPatterns = {"/health_service/dashboard/*",
                "/service/restricted/health_service/*"
        },
        dispatcherTypes = {DispatcherType.REQUEST, DispatcherType.FORWARD}
)
public final class HealthServiceFilter implements Filter {
    private static final String NAME = "HealthServiceFilter";
    ...
    private void doBeforeProcessing(ServletRequest servletRequest, ServletResponse servletResponse) throws IOException, ServletException {
        ...
        if (servletRequest instanceof HttpServletRequest) {
            ...
            if (healthService == null) {
                resp.sendRedirect(resp.encodeRedirectURL(req.getServletContext().getContextPath() + "/health_service/index.xhtml"));
            } else {
                authenticated = true;
            }
        }
